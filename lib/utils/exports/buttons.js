...
const handleButtonClick = client => {
    client.on('interactionCreate', button => {
        ...
        if (button.customId === 'Verify') {
            ...
            if (verified) {return member.send({
        ...
        if (button.customId === 'VerifyUser') {
            ...
            if (message.embeds[0]) {
                ...
                console.log(userID);
                Main.getClient().guilds.fetch('708843719528284262').then(guild => guild.members.fetch(userID).then(member => {
                    if (!member) {
                        message.edit({
                        ...
                        member.send({content: 'You have been verified! Go get your roles and have fun bean! <#716635158228369470> <#738057618215403570>'});
                        message.edit({
                            embeds: [new Discord.MessageEmbed({
