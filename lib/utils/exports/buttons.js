    ...
    client.on('interactionCreate', button => {
        ...
        if (button.customId === 'Verify') {
            ...
            if (verified) {
                ...
                const questions = [
                    'Hello, Where did you join the server from?\n*Please make sure your answers to every question is detailed & clear or we might not verify you.*\n*You have 2mins to answer each question.*',
                    'Why did you join the server?',
                    'Have you been kick/ban from a server, If yes why?',
                    'Are you a furry?',
                    'Tell us alittle about yourself and your fursona if you have one.',
                    'Do you know DSTitan?'
                ];
                const answers = [];
                ...
                button.user.send({
                    ...
                    const appchannel = Main.getClient().channels.cache.get('885240339345055745');
