...
const userJoinAndLeave = (client = Main.getClient()) => {
    ...
    client.on('guildMemberAdd', member => {
        ...
        channel.send({embeds: [embed]}).then(() => {
            chat.send({content: `${member.user} Just Joined\nCreated ${Utils.checkDays(member.user.createdAt)} ${parseInt(Utils.checkDays(member.user.createdAt).split(' ')[0]) < 8 ? '⚠ **New Discord User** ⚠' : ''}`});
        ...
        guild.fetchAuditLogs().then(logs => {
            logs.entries.filter(l => l.action === 'MEMBER_BAN_ADD')
                .forEach(log => {
                    ...
                    channel.send({embeds: [embed]}).then(() => {
                        chat.send({content: `${log.target.tag} Was Banned\nCreated ${Utils.checkDays(log.target.createdAt)}`});
                    });
                });
        });
    });
    ...
    client.on('guildMemberRemove', member => {
        ...
        channel.send({embeds: [embed]}).then(() => {
            chat.send({content:`${member.user.tag} Just Left\nCreated ${Utils.checkDays(member.user.createdAt)}`});
        });
    });
}
