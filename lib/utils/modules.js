...
const userJoinAndLeave = (client = Main.getClient()) => {
    ...
    client.on('guildMemberAdd', member => {
        ...
        channel.send({embeds: [embed]}).then(() => {
            chat.send({content: `${member.user} Just Joined\nCreated <t:${(member.user.createdTimestamp/1000).toFixed()}:R>`}).catch(() => {});
            member.roles.add('760315425861926932').catch(() => {});
            member.roles.add('880852923628789860').catch(() => {});
            member.roles.add('880855152490328114').catch(() => {});
            member.roles.add('880849661148155975').catch(() => {});
            member.roles.add('880848528820273174').catch(() => {});
            setTimeout(() => {
                member.roles.add('769118007552638997').catch(() => {});
            }, 15 * 60 * 1000);
        }).catch(() => {});
    });
        ...
        guild.fetchAuditLogs().then(logs => {
            logs.entries.filter(l => l.action === 'MEMBER_BAN_ADD')
                .forEach(log => {
                    ...
                    channel.send({embeds: [embed]}).then(() => {
                        chat.send({content: `${log.target.tag} Was Banned\nCreated <t:${(log.target.createdTimestamp/1000).toFixed()}:R>${log.target.joinedTimestamp ? ' - Joined <t:'+(log.target.joinedTimestamp/1000).toFixed()+':R>' : ''}`});
                    }).catch(() => {});
                });
        }).catch(() => {});
    });
    ...
    client.on('guildMemberRemove', member => {
        ...
        channel.send({embeds: [embed]}).then(() => {
            chat.send({content:`${member.user.tag} Just Left\nCreated <t:${(member.user.createdTimestamp/1000).toFixed()}:R>${member.joinedTimestamp ? ' - Joined <t:'+(member.joinedTimestamp/1000).toFixed()+':R>' : ''}`});
        }).catch(() => {});
    });
}
 ...
 * @param {number} int 
 */
const errorLog = (error, location, int) => {
    Fetch.default('https://discord.com/api/webhooks/880782479194210315/hp0Cg6E9TidlGHEo0wiQPhTJ48gyEfQzoFTJGBuUh3YYgK3W8iCTwfDDi7nTdFpiUxWs', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            username: 'Log | Titan System',
            embeds: [new Discord.MessageEmbed({
                color: 'BLURPLE',
                description: `\`\`\`diff\nError @ ${location} (${int})\n\`\`\`\n\n\`\`\`bash\n${error}\n\`\`\``
            })]
