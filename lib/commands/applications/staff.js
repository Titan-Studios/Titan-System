...
module.exports = new (class extends CommandConstructor {
    ...
    handle(prefix, command, args, message) {
        ...
        message.react('749843034753204294').catch(error => Modules.errorLog(error, 'staff.js', 1));
        ...
        message.author.send({content:'Starting Staff Application'}).catch(error => Modules.errorLog(error, 'staff.js', 2));
        message.author.send({content:questions[collectCount++]}).then((start) => {
            ...
            collector.on('collect', (i) => {
                if (collectCount < questions.length) {
                    channel.send({content:questions[collectCount++]}).catch(error => Modules.errorLog(error, 'staff.js', 3));
                } else {
                    collector.stop('Completed');
                    channel.send({content:'Thankies! Your staff application has been sent, Please wait as the team review it!'}).catch(error => Modules.errorLog(error, 'staff.js', 4));
                }
            });
            collector.on('end', (collected, reason) => {
                if (reason === 'Completed') {
                    ...
                    appchannel.send({embeds:[embed]}).catch(error => Modules.errorLog(error, 'staff.js', 5));;
                } else {
                    channel.send({content:'There was an error'}).catch(error => Modules.errorLog(error, 'staff.js', 6));;
                }
            })
        }).catch(error => Modules.errorLog(error, 'staff.js', 7));
