...
module.exports = new (class extends CommandConstructor {
    ...
    handle(prefix, command, args, message) {
        ...
        message.author.send({content:'Starting Partnership Application'});
        message.author.send({cotent: questions[collectCount++]}).then((start) => {
            ...
            collector.on('collect', (i) => {
                if (collectCount < questions.length) {
                    channel.send({cotent: questions[collectCount++]});
                } else {
                    collector.stop('Completed');
                    channel.send({content:'Thankies! Your partnership application has been sent!'})
                }
            });
